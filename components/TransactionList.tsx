...
const TransactionList: React.FC<Props> = ({ 
  ...
  return (
    <div className="space-y-6 animate-enter">
      ...
      <div className="bg-slate-900 p-4 sm:p-6 rounded-[2.5rem] border border-slate-800 shadow-sm space-y-4">
        <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4">
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex-1 sm:flex-none">
              ...
              <select 
                ...
                className="w-full bg-slate-800 border border-slate-700 text-slate-100 rounded-xl px-4 py-2 text-[11px] font-bold outline-none focus:border-brand-500"
              >
                {allCategories.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div className="flex-1 sm:flex-none">
              <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-1 block">Frequência</label>
              <select 
            ...
            <div className="flex bg-slate-800 p-1 rounded-xl self-end">
              <button onClick={() => setViewMode('table')} className={`p-2 rounded-lg transition-all ${viewMode === 'table' ? 'bg-slate-700 text-brand-500 shadow-sm' : 'text-slate-500'}`}>
              ...
              <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-slate-700 text-brand-500 shadow-sm' : 'text-slate-500'}`}>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={handleCopyPrevious} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${copyStatus === 'success' ? 'bg-amber-500 border-amber-500 text-white shadow-lg shadow-amber-500/20' : 'bg-slate-900 border-slate-800 text-slate-400 hover:bg-slate-800'}`}>
              ...
              {copyStatus === 'success' ? 'Copiado' : 'Mês Anterior'}
            </button>
            <button onClick={() => { setEditingTransaction(null); setShowModal(true); }} className="flex-1 sm:flex-none px-6 py-3 bg-brand-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-brand-500/20 hover:bg-brand-600 transition-all active:scale-95">
              + Novo
            </button>
          </div>
        </div>
      ...
      <div className="bg-slate-900 rounded-[2rem] border border-slate-800 shadow-sm overflow-hidden">
        {displayTransactions.length === 0 ? (
          <div className="px-8 py-12 text-center text-slate-600 font-medium italic">Nenhum lançamento encontrado para os filtros selecionados.</div>
        ) : viewMode === 'table' ? (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md flex items-center justify-center z-[100] p-4">
          <form onSubmit={handleSubmit} className="bg-slate-900 rounded-[2.5rem] w-full max-w-md p-6 sm:p-10 space-y-6 shadow-2xl animate-enter border border-slate-800">
            ...
            <div className="flex p-1.5 bg-slate-800 rounded-2xl">
              <button type="button" onClick={() => setFormData({...formData, tipo: 'Despesa'})} className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all ${formData.tipo === 'Despesa' ? 'bg-slate-700 text-rose-500 shadow-sm' : 'text-slate-500'}`}>DESPESA</button>
              <button type="button" onClick={() => setFormData({...formData, tipo: 'Receita'})} className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all ${formData.tipo === 'Receita' ? 'bg-slate-700 text-brand-500 shadow-sm' : 'text-slate-500'}`}>RECEITA</button>
            </div>

            <div className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Descrição</label>
                <input type="text" value={formData.descricao} onChange={e => setFormData({...formData, descricao: e.target.value})} placeholder="Ex: Supermercado" required className="w-full p-4 bg-slate-800 border border-slate-700 rounded-2xl focus:border-brand-500 focus:bg-slate-800/50 outline-none font-bold text-slate-100 transition-all" />
              </div>

              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Valor</label>
                <input type="number" step="0.01" value={formData.valor} onChange={e => setFormData({...formData, valor: e.target.value})} placeholder="0,00" required className="w-full p-4 bg-slate-800 border border-slate-700 rounded-2xl focus:border-brand-500 focus:bg-slate-800/50 outline-none font-black text-3xl text-slate-100" />
              </div>
              ...
              <div className="space-y-1">
                ...
                <select value={formData.frequencia} onChange={e => setFormData({...formData, frequencia: e.target.value as TransactionFrequency})} className="w-full p-4 bg-slate-800 border border-slate-700 rounded-2xl outline-none text-xs font-black text-slate-300 focus:border-brand-500 focus:bg-slate-800/50 transition-all">
